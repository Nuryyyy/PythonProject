...
def quiz_1():
    score = 0
    shuffle(True_False) #shufle the items, 
    ...
    os.system("pause")
    return total_score
def quiz_2(): #for the multiple choice
    total_score = 0
    score = 0 # per item
    question_num = 6
...
intro()
overall_points = quiz_1() + quiz_2()
if overall_points <= 9:
        print(f"\nFinal Score: {overall_points}/15 \nFAILED! To do well on the quiz, you should study more.")
elif overall_points == 15:
        print(f"\nFinal Score: {overall_points}/15 \nPERFECT! Such a brilliant mind.")
elif 10 > overall_points >= 14: #10-14
